...
{
    public class PosSalesReportController : BaseController
    {
        IApiRequest _apiRequest;
        public PosSalesReportController(IApiRequest apiRequest)
        {
        ...
        {
            var result = await _apiRequest.GetPosSaleReport(CompanyId, model.date , model.date2);
            return Json(result);
        }
    }
}
