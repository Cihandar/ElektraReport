...
{
    public class PosDiscountReportController : BaseController
    {
        IApiRequest _apiRequest;
        public PosDiscountReportController(IApiRequest apiRequest)
        {
        ...
        public async Task<IActionResult> GetReport(RequestModel model)
        {
            ...
            var result = await _apiRequest.GetPosDiscountReport(CompanyId, model.date , model.date2);
            return Json(result);
        }
    }
}
